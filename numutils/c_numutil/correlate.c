/************************************************************************/
/* Correlate.c -							*/
/*  This program correlates to complex or real vectors.			*/
/*  (Defaults to frequency domain convolution for efficiency.)		*/
/* 									*/
/* Options:								*/
/*	-t	- Perform Time Domain Correlation. (slower)		*/
/*	-f	- Perform Frequency Domain Correlattion. (default)	*/
/* 									*/
/* Example Usage:							*/
/*	correlate infile1  infile2  outfile  			  	*/
/* 								  	*/
/* File Types:								*/
...
char *argv[];
{
 ...
 { /*arg*/
  ...
  if (argv[arg][0]=='-')
   { /*option*/
     ...
     else {printf("ERROR correlate: Unknown command-line option %s\n", argv[arg]); exit(1);}
   } /*option*/
  else
   { /*file_name*/
    if (k==0)  
    ...
    if (k==2)
     { strcpy(fname3,argv[arg]);  
       if (strcmp(fname1,fname3)==0) {printf("ERROR correlate: Can't over-write source.\n"); exit(1);}
       if (strcmp(fname2,fname3)==0) {printf("ERROR correlate: Can't over-write source.\n"); exit(1);}
       k = k + 1; 
     }
    else
     {printf("ERROR correlate: Too many file-names on command line.\n"); exit(1);}
   } /*file_name*/

  arg = arg + 1;
 } /*arg*/

 if (k<2) { printf("ERROR correlate: Missing input file on command line.\n"); exit(1); }
 if (infileA->fpt == 0) {printf("ERROR correlate: Can't open first input file '%s'.\n",fname1); exit(1);}
 if (infileB->fpt == 0) {printf("ERROR correlate: Can't open second input file '%s'.\n",fname2); exit(1);}
 if (infileA->kind != infileB->kind) {printf("ERROR correlate: Input files have mismatching types.\n"); exit(1);}
 ...
 if (k<=2)  /* No output file specified, so make default name by appending suffix. */
  { 
    ...
    if ((strcmp(fname1,fname3)==0) || (strcmp(fname2,fname3)==0))
	 {printf("ERROR correlate: Can't over-write source.\n"); exit(1);}
  }

 printf("	(Writing output to %s)\n", fname3 );
 outfile = file_open_write( fname3, infileA->kind, infileA->dim1, infileA->dim2 );
 if (outfile->fpt==0) {printf("ERROR correlate: Can't open output file '%s'.\n",fname3); exit(1);}
   ...
   if (word[0]!='\0')
    {
     if (infileA->kind=='R')
      {
        Next_Word(line,trsh,word," 	,()");
        if (sscanf(word,"%lf",&(a.r))!=1) printf("ERROR correlate: Reading real '%s' on line %d.\n",word,i+1);
	a.i = 0.0;
      }
     else
      {
       if (sscanf(word,"%lf",&(a.r))!=1) printf("ERROR correlate: Reading real '%s' on line %d.\n",word,i+1);
       Next_Word(line,trsh,word," 	,()");
       if (sscanf(word,"%lf",&(a.i))!=1) printf("ERROR correlate: Reading imaginary '%s' on line %d.\n",word,i+1);
      }
     x[k] = a;
     k = k + 1;
    }
 ...
 if (fdomain)
  { /*fdomain*/
    ...
    for (i=0; i<N; i++)  w[i] = CMULT( x[i], CONJG( y[i] ) );
