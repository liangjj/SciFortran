...
/*  by computing the complex-magnitude of each term.			*/
/* 									*/
/* Options:								*/
/*	(no options)							*/
/* 									*/
...
char *argv[];
{
 ...
 { /*arg*/
  ...
  if (argv[arg][0]=='-')
   { /*option*/
     {printf("ERROR cmag: Unknown command-line option %s\n", argv[arg]); exit(1);}
   } /*option*/
  else
   { /*file_name*/
    if (k==0)  
    ...
    if (k==1)
     { strcpy(fname2,argv[arg]);  
       if (strcmp(fname1,fname2)==0) {printf("ERROR cmag: Can't over-write source.\n"); exit(1);}
       k = k + 1; 
     }
    else
     {printf("ERROR cmag: Too many file-names on command line.\n"); exit(1);}
   } /*file_name*/

  arg = arg + 1;
 } /*arg*/

 if (k==0) { printf("ERROR cmag: No input file on command line.\n"); exit(1); }
 if (infile->fpt == 0) {printf("ERROR cmag: Can't open input file '%s'.\n",fname1); exit(1);}
 if (infile->kind!='C') {printf("ERROR cmag: Input file is not Complex.\n"); exit(1);}
 ...
 if (outfile->fpt==0) {printf("ERROR cmag: Can't open output file '%s'.\n",fname2); exit(1);}
 ...
 while (! feof( infile->fpt ))
  {
   ...
   if (word[0]!='\0')
    {
     if (sscanf(word,"%lf",&x)!=1) printf("ERROR cmag: Reading real '%s' on line %d.\n",word,i+1);
     Next_Word(line,trsh,word," 	,()");
     if (sscanf(word,"%lf",&y)!=1) printf("ERROR cmag: Reading imaginary '%s' on line %d.\n",word,i+1);
     a.r = x;  a.i = y;
     y = CMAG( a );
